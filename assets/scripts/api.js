...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlkeWp2bW1sZHRkdnBrbGt6cmdyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTMyODg4NTgsImV4cCI6MjA2ODg2NDg1OH0.DB-6F-joVK-oaFCw9jBoiqXlPFAMzbzh4TLE2EdD_b0';
...
async function fetchTalentData(talentName) {
   ...
   iconElement.src = "/assets/images/icons/talent/" + data.icon + ".png" || "/assets/images/icons//question.png";
      ...
      if (entries.length > 0) {
            ...
            bonus1Element.textContent = `+${value} ${key}`;
      } else {
      ...
      if (entries.length > 1) {
            ...
            bonus2Element.textContent = `+${value} ${key}`;
   ...
   cardColorElement.style.backgroundColor = data.rarity === `var(--color-card-${data.rarity.toLowerCase()})`;
